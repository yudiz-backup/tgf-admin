...
import { changeNLHStatus, changePLOStatus } from 'query/poker/poker.query'

const NLHPokerList = ({ table, index, onDelete, location }) => {
    const navigate = useNavigate()
    const query = useQueryClient()

    // Status
    const { mutate: statusMutation, } = useMutation(changeNLHStatus, {
        onSuccess: (response) => {
    ...
    const handleConfirmStatus = (e, id) => {
        if (location?.pathname?.endsWith('PLO')) {
    ...
    return (
        <>
            <tr key={table._id} className={table.eStatus === 'd' && 'deleted-user'} >
                ...
                <td>{table?.nAnte || 0}</td>
                <td>{table?.nSmallBlind || 0}</td>
                <td>{table?.nBigBlind || 0}</td>
