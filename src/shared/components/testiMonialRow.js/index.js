  ...
  return (
    <>
      <tr key={user._id} className={user.eStatus === "d" && "deleted-user"}>
        ...
        <td className="date-data-field">{user?.eType === "A" ? "About us" : "Home Page"}</td>
        <td className="date-data-field">{user?.sDesignation}</td>
        <td className="date-data-field">{user?.sReview}</td>
          ...
          <Dropdown className="dropdown-datatable">
            ...
            <Dropdown.Menu className="dropdown-datatable-menu">
              <Dropdown.Item
                ...
                onClick={() => navigate(route.editTestimonial(user._id))}
              >
