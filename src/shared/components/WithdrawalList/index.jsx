...
const WithdrawalList = ({ withdrawal, index }) => {
  ...
  return (
    <>
      <tr key={withdrawal?._id}>
        ...
        <td>
          <TriggerTooltip className='user single-line' data={withdrawal?.sUserName || '-'} display={withdrawal?.sUserName || '-'} onClick={() => navigate(route.viewUser(withdrawal?.iUserId), { state: 'withdrawal' })} />
        </td>
        <td>{withdrawal?.sEmail || '-'}</td>
        <td>{withdrawal?.sMobile || '-'}</td>
