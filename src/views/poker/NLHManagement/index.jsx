...
import { deleteNLH, getNLHList } from 'query/poker/poker.query'
...
import { NLHPokerListColumn } from 'shared/constants/TableHeaders'
import { appendParams, parseParams } from 'shared/utils'
import NLHPokerList from 'shared/components/NLHPokerList'
...
const NLHManagement = () => {
  ...
  const [columns, setColumns] = useState(getSortedColumns(NLHPokerListColumn, parsedData))
  const [modal, setModal] = useState({ open: false, type: '' })

  // List
  const { isLoading, isFetching, data } = useQuery(['NLHList', requestParams], () => getNLHList(requestParams), {
    select: (data) => data.data.data,
  })

  // DELETE USER
  const { mutate } = useMutation(deleteNLH, {
    onSuccess: (res) => {
  ...
  useEffect(() => {
    document.title = 'NLH | Poker | PokerGold'
  }, [])
        ...
        buttons={[
          {
            ...
            btnEvent: () => navigate(route.addNLHTable)
          }
        ]}
      />
      <div>
        ...
        >
          ...
          <Drawer isOpen={modal.type === 'filter' && modal.open} onClose={() => setModal({ open: false, type: '' })} title='NLH Poker List Filter'>
            <NLHListFilter
