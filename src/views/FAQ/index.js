...
import { deleteCategory, getAllFaqCategory } from "query/faq/faq.api";
...
import FaqCategoryList from "shared/components/FaqCategoryListRow";
import CustomModal from "shared/components/Modal";
import TopBar from "shared/components/Topbar";
import UserFilters from "shared/components/UserListFilter";
import { route } from "shared/constants/AllRoutes";
import { FaqCategoryColumn } from "shared/constants/TableHeaders";
...
function FaqManagement() {
  ...
  const [columns, setColumns] = useState(
    getSortedColumns(FaqCategoryColumn, parsedData)
  );
  ...
  const { isLoading, isFetching, data } = useQuery(
    ["FaqCategoryList", requestParams],
    () => getAllFaqCategory(requestParams),
    {
        ...
        buttons={[
          {
            ...
            btnEvent: () => navigate(route.add_category),
          },
        ]}
      />
      <div>
        <DataTable
          ...
          totalRecord={data && (data?.data?.length || 0)}
